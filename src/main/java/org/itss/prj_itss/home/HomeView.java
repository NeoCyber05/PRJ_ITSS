...
public class HomeView {
    ...
    public HomeView(Navigator navigator, DAOFactory dao) {
    ...
    private HBox buildWelcomeSection() {
        ...
        eyebrow.setStyle(
            ...
            "-fx-text-fill: #CCFBF1; -fx-background-radius: 999;" +
            "-fx-padding: 5 12; -fx-font-size: 11px; -fx-font-weight: bold;"
        );
        Label title = new Label("Bộ phận đặt hàng quốc tế");
        title.setWrapText(true);
        title.setStyle("-fx-font-size: 28px; -fx-font-weight: bold; -fx-text-fill: white;");
        Label subtitle = new Label("Trang này là điểm vào nhanh để xem yêu cầu đã nhận, quản lý site và theo dõi đơn hàng đã tạo.");
        subtitle.setWrapText(true);
        subtitle.setStyle("-fx-font-size: 13px; -fx-text-fill: #D6F4F1;");

        HBox actionRow = new HBox(12);
        actionRow.getChildren().addAll(
            buildHeroButton("Yêu cầu đã nhận", true, () -> navigator.showView("received-requests")),
            buildHeroButton("Đơn hàng đã tạo", false, () -> navigator.showView("orders"))
        );
        ...
        right.setStyle(
            ...
            "-fx-background-radius: 18; -fx-border-color: rgba(255,255,255,0.14);" +
        ...
        int pendingCount = (int) requestDAO.findAll().stream()
            .filter(r -> "Chờ xử lý".equals(r.getStatus())).count();
        int siteCount = siteDAO.countAll();
        int orderCount = orderDAO.findAll().size();

        right.getChildren().addAll(
            rightTitle,
            buildSummaryLine(String.format("%02d", pendingCount), "yêu cầu chờ xử lý"),
            buildSummaryLine(String.format("%02d", siteCount), "site đang hoạt động"),
            buildSummaryLine(String.format("%02d", orderCount), "đơn hàng đã tạo")
        );
    ...
    private Button buildHeroButton(String text, boolean primary, Runnable action) {
        ...
        if (primary) {
            button.setStyle("-fx-background-color: white; -fx-text-fill: #0F172A; -fx-background-radius: 999; -fx-padding: 10 18; -fx-font-size: 13px; -fx-font-weight: bold; -fx-cursor: hand;");
        } else {
            button.setStyle("-fx-background-color: rgba(255,255,255,0.08); -fx-text-fill: white; -fx-background-radius: 999; -fx-border-color: rgba(255,255,255,0.22); -fx-border-radius: 999; -fx-border-width: 1; -fx-padding: 10 18; -fx-font-size: 13px; -fx-font-weight: bold; -fx-cursor: hand;");
        }
        return button;
    }
    ...
    private HBox buildSummaryLine(String value, String text) {
        ...
        valueLabel.setStyle("-fx-background-color: rgba(255,255,255,0.12); -fx-text-fill: white; -fx-background-radius: 12; -fx-padding: 6 10; -fx-font-size: 13px; -fx-font-weight: bold;");
    ...
    private VBox buildQuickCard(String token, String title, String description, String accent, String viewId) {
        ...
        card.setOnMouseClicked(e -> navigator.showView(viewId));
    ...
    private VBox buildPendingSection() {
        ...
        HBox header = buildSectionHeader("Công việc cần xử lý", "Mở danh sách", () -> navigator.showView("received-requests"));
    ...
    private HBox buildPendingRow(String requestId, String owner, String deadline, String itemCount) {
        ...
        button.setOnAction(e -> navigator.showView("received-requests"));
        button.setStyle("-fx-background-color: #0F172A; -fx-text-fill: white; -fx-background-radius: 999; -fx-padding: 8 14; -fx-font-size: 12px; -fx-font-weight: bold; -fx-cursor: hand;");
    ...
    private HBox buildSectionHeader(String titleText, String actionText, Runnable action) {
        ...
        actionButton.setStyle("-fx-background-color: transparent; -fx-text-fill: #0F766E; -fx-padding: 0; -fx-font-size: 12px; -fx-font-weight: bold; -fx-cursor: hand;");
        header.getChildren().addAll(title, spacer, actionButton);
        return header;
    }

    private String panelStyle() {
        return "-fx-background-color: white; -fx-background-radius: 20; -fx-border-color: #DCE5F1; -fx-border-radius: 20; -fx-border-width: 1; -fx-effect: dropshadow(gaussian, rgba(15,23,42,0.05), 14, 0, 0, 4);";
    }
